...
export default function LocationsPage() {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <Field label="Location name">
        <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
          placeholder={`e.g. ${restaurant?.name} — Montrose`} style={inputStyle}
      ...
      <div style={{ borderTop: '1px solid var(--border)', paddingTop: 16, marginTop: 4 }}>
        <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 12 }}>Hours</div>
        {hours.map((row, i) => (
  ...
  return (
    <div style={{ padding: '24px 28px 32px' }}>
      <PageHeader
        title="Locations"
        subtitle="Manage your storefronts — each with its own hours, links and address"
        action={<Button variant="primary" onClick={openAdd}>+ Add Location</Button>}
      />

      {locations.length === 0 && (
        <Card style={{ textAlign: 'center', padding: 40 }}>
          <div style={{ fontSize: 32, marginBottom: 12 }}>📍</div>
          <div style={{ fontSize: 15, fontWeight: 500, marginBottom: 6 }}>No locations yet</div>
          <div style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 20 }}>Add your first storefront to get started</div>
          <Button variant="primary" onClick={openAdd}>+ Add Location</Button>
        </Card>
      )}
        ...
        {locations.map((loc, i) => (
          <Card key={loc.id} style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '14px 20px', background: '#FAFAF8', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontSize: 15, fontWeight: 500 }}>{loc.name || `Location ${i + 1}`}</div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <Button size="sm" variant="danger" onClick={() => handleDelete(loc.id)}>Delete</Button>
              </div>
            </div>
            <div style={{ padding: '16px 20px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
              <div>
                <div style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 }}>Contact</div>
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: loc.location_links?.[0]?.order_url ? 'var(--gold-dark)' : 'var(--muted)' }}>
      ...
      <Modal open={addModal} onClose={() => setAddModal(false)} title="Add Location"
        footer={<>
          ...
          <Button variant="primary" onClick={() => handleSave(false)} disabled={!form.name || saving}>{saving ? 'Saving...' : 'Add Location'}</Button>
        </>}>
        <LocationForm />
      </Modal>

      <Modal open={!!editModal} onClose={() => setEditModal(null)} title={`Edit — ${editModal?.name || 'Location'}`}
        footer={<>
